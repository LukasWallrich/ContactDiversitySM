...
paste("N=",round(sum(svytable(~respid, design = ac_svy)))) %>% print()
round(prop.table(svytable(~sex, design = ac_svy)), 3) %>% print()
svy_mi_mean_sd(ac_svy_MI, "age") %>% print()
round(prop.table(svytable(~eastwest, design = ac_svy)), 3) %>% print()
