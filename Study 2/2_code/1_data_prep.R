...
ac %<>% mutate(nb_scoreN = ifelse(nb_scoreY==13, 0, 100 - (nb_scoreN-1)*4/49*100),
               nb_scoreY = (nb_scoreY-1)*4/49*100)
...
ac.w$variables$mp15bc <- rNuggets::cut_p(ac.w$variables$mp15b %>% as.numeric(), rev(ac.w$variables$mp15a %>% table() %>% prop.table() %>% as.numeric()), fct_levels = rev(levels(as_factor(ac.w$variables$mp15a))[-(1:2)]))
