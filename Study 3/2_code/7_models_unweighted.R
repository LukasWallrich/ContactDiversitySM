...
NAME <- '7_models_unweighted' ## Name of the R file goes here (without the file extension!)

# ------------
# Sources
# ------------
if (!require("pacman")) install.packages("pacman")
pacman::p_load(tidyverse, magrittr, here, foreign, survey, srvyr, lavaan, lavaan.survey)
...
imp_svy <- svydesign(ids = ~1, weights = ~1, data = mitools::imputationList(imp_long_list_with_dummy_sd))

mod_imputed <- lavaan.survey::lavaan.survey(mod_complete, imp_svy)


ind_CIs <- semTools::monteCarloCI(mod_imputed, nRep = 2e4) %>% tibble::rownames_to_column("name") %>%
  rename(est.std = est) %>% filter(str_detect(name, "^ind_|^total|^pair_"))

ind_p <- semTools::monteCarloCI(mod_imputed, return.samples = TRUE, nRep = 2e4) %>% select(matches("^ind_|^total|^pair_")) %>%
...
res_tbl <- res %>%
  ...
  rNuggets::gt_apa_style()

res_tbl

res_tbl  %>% gt::gtsave(filename = here(pipeline, "out", "mediation_unweighted.html"))

graph_parameters <- parameterestimates(mod_imputed, ci = TRUE) %>% filter(nchar(label)>0)
